...
async function seedCategories() {
    ...
    for (const cat of categories) {
        const result = await (0, database_1.query)(`INSERT INTO categories (name, type, icon, color, is_system)
       VALUES ($1, $2, $3, $4, TRUE)
       ON CONFLICT (name) DO UPDATE SET type = EXCLUDED.type
       RETURNING id, name`, [cat.name, cat.type, cat.icon, cat.color]);
...
async function seedTestData() {
    try {
        ...
        const password_hash = await password_1.PasswordUtils.hash('Test123!');
        await (0, database_1.query)(`INSERT INTO users (name, email, password_hash, phone)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (email) DO NOTHING`, ['Test User', 'test@example.com', password_hash, '+8801712345678']);
        const userResult = await (0, database_1.query)('SELECT * FROM users WHERE email = $1', ['test@example.com']);
        ...
        for (const transaction of transactions) {
            await Transaction_1.TransactionModel.create(transaction);
        }
