...
class AnalyticsService {
    ...
    `;
      ...
      LIMIT 1
    `;
        const topCategoryResult = await (0, database_1.query)(topCategorySql, [userId, month, year]);
    ...
    static async getTopMerchants(userId, limit = 10, startDate, endDate) {
        let sql = `
      SELECT 
        t.merchant,
        SUM(t.amount) as total_amount,
    ...
    `;
        const result = await (0, database_1.query)(sql, [userId, startDate, endDate]);
        const row = result.rows[0];
        const totalIncome = parseFloat(row.total_income);
        const totalExpenses = parseFloat(row.total_expenses);
